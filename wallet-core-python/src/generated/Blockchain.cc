...
// Create PyBlockchain from enum TWBlockchain. It returns the same PyBlockchain instance
// for the same enum TWBlockchain value.
...
bool PyInit_Blockchain(PyObject *module) {
  ...
  for (const Constant& constant : constants) {
    PyDict_SetItemString(dict, constant.name, PyBlockchain_FromTWBlockchain(constant.value));
  }
